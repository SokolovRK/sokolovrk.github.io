...
$('.accordion-header').on('click', function () {
	...
	if (!$(this).hasClass('active')) {
		activeHeader.next().slideUp(300).removeClass('active');
		activeHeader.removeClass('active');
		$(this).addClass('active').next().slideToggle(4000).addClass('active');
	}
});
...
$.ajax({
	...
	if (data.work_count == 0) {
		$('.row.books').addClass('justify-content-center my-40').html('<h3>No books found</h3>');
		return false;
	}
	...
	$.each(data.works, function (index, bookInfo) {
		...
		if (!bookInfo.hasOwnProperty('availability') || (bookInfo.availability.status === 'error')){
